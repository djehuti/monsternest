...
control {
    ...
    const uword ABXY_MASK = $FF ^ (A & B & X & Y)
    const uword SSEL_MASK = $FF ^ (Start & Select)
    const uword DPAD_MASK = $FF ^ (Right & Left & Down & Up)
}
...
tiles {
    const ubyte Asterisk = 42
    const ubyte Space = 20
    const ubyte Egg = 87
    const ubyte Pi = 94
}
